...
public interface IList<E> extends Iterable<E> {
    ...
    default boolean remove(Object o) // удалять элемент по его индексу, если он есть
    {
        int index = indexOf(o);
        if (index < 0) { // когда объекта нет в списке
            return false;
        }
        removeByIndex(index); // удалили индекс, и объект "пропал" из списка
        return true;
    }

    boolean add(int index, E element); // добавить (вставить) элемент по индексу

    E removeByIndex(int index); // удалить элемент из списка по индексу
    ...
    int lastIndex(Object o); // ищем индекс элемента с конца списка
}
